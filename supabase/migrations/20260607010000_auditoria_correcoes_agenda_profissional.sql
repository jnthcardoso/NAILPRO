-- AUDITORIA (07/06) — correções de segurança e performance da agenda_profissional.

-- [CRÍTICO 1] Vazamento de colunas internas ao público.
-- anon lia TODAS as 15 colunas (user_id, salao_id, membro_id, google_conectado, timestamps).
-- Restringe anon às colunas públicas — mesmo padrão da `configuracoes`.
revoke select on public.agenda_profissional from anon;
grant select (slug, nome_exibicao, whatsapp, servicos_padrao, horario_inicio,
              horario_fim, duracao_atendimento, dias_semana, agenda_publica_ativa)
  on public.agenda_profissional to anon;

-- [ALERTA] FK agendamentos.profissional_id sem índice (as RPCs públicas filtram por ele).
create index if not exists idx_agendamentos_profissional on public.agendamentos(profissional_id);

-- [PERF/SUGESTÃO] Políticas dos logados só para 'authenticated' (some warning de múltiplas
-- policies p/ anon), (select auth.uid()) p/ não reavaliar por linha, e profissional lê só a
-- própria linha (dona/recepção leem as do salão). A política anon-por-slug é mantida.
drop policy if exists agenda_prof_select on public.agenda_profissional;
create policy agenda_prof_select on public.agenda_profissional for select to authenticated
  using (user_id = (select auth.uid()) or (select public.meu_papel()) in ('dona','recepcionista'));

drop policy if exists agenda_prof_insert on public.agenda_profissional;
create policy agenda_prof_insert on public.agenda_profissional for insert to authenticated
  with check (user_id = (select auth.uid())
              and salao_id = (select public.meu_salao_id())
              and membro_id = (select public.meu_membro_id()));

drop policy if exists agenda_prof_update on public.agenda_profissional;
create policy agenda_prof_update on public.agenda_profissional for update to authenticated
  using (user_id = (select auth.uid()))
  with check (user_id = (select auth.uid()));

-- [CRÍTICO 2] Ordem de resolução do slug = "sequestro" de agendamento.
-- Antes, as RPCs resolviam a MANICURE antes do SALÃO; uma manicure podia cadastrar o slug
-- de um salão existente e desviar os agendamentos dele pra ela. Agora o SALÃO (configuracoes)
-- é resolvido PRIMEIRO; a manicure só resolve se o slug não pertencer a nenhum salão.
...
BEGIN
  -- SALÃO primeiro; manicure só se o slug não for de nenhum salão.
