...
export default function Clientes() {
  ...
  return (
    <div style={s.page}>
      ...
      {filtradas.length > 0 && (
        <>
        {/* ── Lista mobile: cards (Nome+badge | WA+tel / Histórico | Próximo) ── */}
            ...
            return (
              <div key={c.id} style={{ ...s.mobileRow, background: rowBg }} onClick={() => navigate(`/app/clientes/${c.id}`)}>
                ...
                <div style={s.mobileLeft}>
                  {/* Nome + badge rente */}
                  <div style={s.mobileNomeRow}>
                    <span style={s.mobileNome}>{c.nome}</span>
                    <span style={{ ...s.mobileStatusBadge, background: statusBg, color: statusColor }}>{statusLabel}</span>
                  </div>
                  {/* WhatsApp + número */}
                  <button
                    style={s.mobileWaBtn}
                    onClick={e => { e.stopPropagation(); handleWhatsApp(e, c) }}
                  >
...
const s = {
  ...
  // ── Lista mobile de clientes (cards) ──
  mobileHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 14px', background: 'var(--surface2)', borderBottom: '1px solid var(--border)' },
  mobileHeadLabel: { fontSize: 9, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.5px' },
  mobileRow: { display: 'flex', alignItems: 'stretch', justifyContent: 'space-between', padding: '11px 14px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background 0.1s', gap: 10 },
  mobileLeft: { flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 5 },
  mobileNomeRow: { display: 'flex', alignItems: 'center', gap: 5, minWidth: 0, overflow: 'hidden' },
  mobileNome: { fontSize: 13, fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 1, minWidth: 0 },
